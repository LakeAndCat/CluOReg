# -*- encoding: utf-8 -*-
'''
@File    :   CluORegAddMixupv2.py    
@Contact :   guzhouweihu@163.com

@Modify Time      @Author        @Version    @Desciption
------------      -----------    --------    -----------
2020/12/23 15:24   guzhouweihu      1.0         None
'''
...
from helper.mixupUtils import mixup_data, mixup_criterion, mixup_criterion_pseudo_distillation
...
def train_distill(epoch, model, ema_model, cluster, cluster_optimizer, cTok, cTok_optimizer, optimizer, train_loader, criterion, pseudo_criterion, args):
    ...
    for idx, (input, target) in enumerate(train_loader):
        with torch.autograd.set_detect_anomaly(True):
            ...
            # mixup data
            mixup_inputs, mixup_targets_a, mixup_targets_b, lam = mixup_data(input, target, args, args.mixup_alpha)

            # ===================forward=====================
            # outputs, features = model(input)
            mixup_outputs, mixup_features = model(mixup_inputs)
            outputs, features = model(input)
            # loss = criterion(outputs, target)
            mixup_loss = mixup_criterion(criterion, mixup_outputs, mixup_targets_a, mixup_targets_b, lam)
            acc1 = accuracy(mixup_outputs, target, topk=(1,))
            losses.update(mixup_loss.item(), input.size(0))
            ...
            if cluster is not None:
                with torch.no_grad():
                    ...
                    # ema_mixup_outputs, ema_mixup_features = ema_model(mixup_inputs)
                ...
                mixup_loss += MEncoder_KD_loss * args.MEncoder_weight + pseudo_loss * args.pseudo_weight
